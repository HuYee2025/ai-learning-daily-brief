...
这个项目不是把 AI 新闻重新摘要一遍。它的价值在于：从“今天发生了什么”，继续推进到“哪些内容有依据、哪些只是观点、哪些是智能体的推断，以及这些信息对正在做的项目有什么用”。

普通简报通常停在信息整理：列标题、压缩内容、给几个链接。这个 Skill 的分水岭是下面这类追问：

```text
你研究一下这篇（原文链接），按章节提取核心观点，并为每个核心观点给出原文依据，标注哪些内容是事实、哪些是作者观点、哪些是你的推断。告诉我那些信息对我们现在的项目工作和流程、每日简报有什么帮助。
```
...
1. 确定性的结构化表达
   ...
   每条重要信息都要拆成 `事实`、`作者观点`、`我的推断`，并尽量给出原文依据。这样读者能看清：哪些能从原文核对，哪些是作者判断，哪些是智能体结合上下文做出的迁移推断。

2. 面向项目的落地建议

   本地 Agent 了解用户正在推进的项目、工作流和内容方向，所以简报不是“收藏夹”，而是会把信息映射到学习动作、项目实验、流程调整或公众号/短视频选题上。闭环在这里形成。

一句话说：它不是新闻简报，而是面向个人项目系统的 AI 学习决策简报。
